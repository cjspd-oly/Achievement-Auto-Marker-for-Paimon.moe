...
def smart_merge_imports(current_data: dict, uploaded_data: dict) -> dict:
    for key, val in uploaded_data.items():
        if isinstance(val, dict):
            if key not in current_data:
                current_data[key] = {}
            for sub_key, sub_val in val.items():
                if isinstance(sub_val, dict):
                    if sub_key not in current_data[key] or not isinstance(current_data[key][sub_key], dict):
                        current_data[key][sub_key] = {}
                    for subsub_key, subsub_val in sub_val.items():
                        if subsub_val:
                            current_data[key][sub_key][subsub_key] = True
                        elif subsub_key not in current_data[key][sub_key]:
                            current_data[key][sub_key][subsub_key] = False
                else:
                    if sub_val:
                        current_data[key][sub_key] = True
                    elif sub_key not in current_data[key]:
                        current_data[key][sub_key] = False
        else:
            if val:
...
def match_and_update_import(config):
    ...
    for title in titles:
        ...
        if best_match:
            ...
            if "achievement" not in import_data or not isinstance(import_data["achievement"], dict):
                import_data["achievement"] = {}
            if "0" not in import_data["achievement"] or not isinstance(import_data["achievement"]["0"], dict):
            ...
            if str(matched_id) in checklist and isinstance(checklist[str(matched_id)], dict):
                if "0" in checklist[str(matched_id)] and not checklist[str(matched_id)]["0"]:
