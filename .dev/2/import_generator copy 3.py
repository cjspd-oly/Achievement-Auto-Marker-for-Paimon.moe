...
def create_default_config(config_path: str, main_config_path: str):
    ...
    if os.path.exists(user_partial_dir):
        partial_files = [
            os.path.join(user_partial_dir, f)
            for f in os.listdir(user_partial_dir)
            if f.lower().endswith(".json")
        ]
        if partial_files:
            partial_file_path = max(partial_files, key=os.path.getctime)

    if partial_file_path:
        uploaded_file_path = partial_file_path
        merge_mode = True
        print(f"Using user partial file: {uploaded_file_path}")
    else:
        if os.path.exists(uploads_dir):
            ...
            if upload_files:
                uploaded_file_path = max(upload_files, key=os.path.getctime)
                merge_mode = True
                print(f"Using last uploaded file: {uploaded_file_path}")
    ...
    if not uploaded_file_path:
        ...
        if os.path.exists(fallback_file):
            ...
            merge_mode = True
            print(f"Fallback: created upload from raw.json -> {uploaded_file_path}")
        else:
            raise FileNotFoundError("No user uploads, fallback, or raw.json found to proceed.")
        
    # Also create a symbolic/latest file to trigger Choice 2 next time
    latest_file = os.path.join(merged_dir, "latest.json")
    with open(latest_file, "w", encoding="utf-8") as f:
        json.dump(raw_data, f, indent=4)

    # Update uploaded_file_path to latest.json (choice 2 next run)
    ...
    default_config = {
        ...
        "settings": {"threshold": 90, "verbose": True, "merge_uploads": merge_mode},
    }
